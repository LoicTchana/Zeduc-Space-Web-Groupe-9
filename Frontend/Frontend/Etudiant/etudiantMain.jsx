...
const CardComponent = ({ imageSrc, smallImage1, smallImage2, price, dishName, description, additionalDescription, onAddToCart }) => (
  <div className="card" style={{ width: '200px', height: '300px', borderRadius: '20px', border: 'none', backgroundColor: '#D9D9D9', position: 'relative', overflow: 'hidden' }}>
    ...
    <img src={smallImage2} alt="Small 2" style={{ width: '30px', height: '20px', position: 'absolute', top: '50px', right: '10px' }} />
    ...
    <div className="card-body" style={{ padding: '120px 10px 10px 10px', textAlign: 'center' }}>
      ...
      <Button variant="primary" style={{backgroundColor: '#D4B35E', borderColor: '#D4B35E',display: 'block', 
    margin: '0 auto',width:'150px'}} onClick={onAddToCart}>Ajouter au panier</Button>
    </div>
  </div>
);
...
const ShoppingCart = ({ cartItems, onConfirm, onRemove }) => {
  ...
  return (
    <div style={{ border: '1px solid #ccc', borderRadius: '10px', padding: '10px', width: '450px', height:'500px', position: 'absolute', right: '60px', top: '120px', backgroundColor: '#F3F3F3',overflow: 'hidden' }}>
      <h5>Mon panier</h5>
      <hr />
      <div>
  ...
  const cardData = [
    {
      ...
      smallImage1: imagelike,
      smallImage2: imagedrop,
      price: '24',
      dishName: 'Plat 1',
    ...
    <Container fluid>
      <Row>
        <Col md={8}>
          ...
          {/* Boutton de reclamations */}
          <Button variant='danger' style={{marginTop:'200px',borderRadius:'20px', fontWeight:'bold'}} onClick={() => navigate('/Reclamations')} >Faire une Reclamation</Button> 
